...
export default function NotificationItem({
  ...
  // Improved date formatting with better relative time
        ...
        return {
          ...
          content: (
            <span className="text-orange-700">
              {notification.metadata?.message || "Payment pending"}
            </span>
          ),
        };
            ...
            <span className="text-gray-700">
              {notification.metadata
                ? typeof notification.metadata === "string"
                  ? notification.metadata
                  : notification.metadata.message || "New notification"
                : "New notification"}
            </span>
          ),
        };
  ...
  const getUserInitials = useMemo(() => {
    if (!notification.sender?.name) {
      return notification.sender_id?.charAt(0)?.toUpperCase() || "?";
    }
